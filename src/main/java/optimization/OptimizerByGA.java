...
public class OptimizerByGA {
     ...
     */
    public OptimizerByGA(parentModel model, int populationSize, int evolveTime, double[] target, double[] initPara, double threshold) {
        this.model = model;
    ...
    private void addListener(GeneticAlgorithm<MyVector, Double> ga) {
        ...
        ga.addIterationListener(new IterartionListener<MyVector, Double>() {
            ...
            public void update(GeneticAlgorithm<MyVector, Double> ga) {
                ...
                System.out.printf("%s\t%s\t%s%n", iteration, String.format("%.2f",bestFit), best);
    ...
    public static class MyVectorFitness implements Fitness<MyVector, Double> {
        ...
        public Double calculate(MyVector chromosome) {
            ...
            double[] q = model.paraOptimize(v);
//            System.out.println(Arrays.toString(q));
            for (int i = 0; i < q.length; i++) {
                delta += this.sqr(q[i] - this.target[i]);
            }
            delta = Math.sqrt(delta)/q.length;
            return (delta);
        }
